...
# ============================================================================
""" Focal Transformer v2 high version, like 1.6.0+ """
...
def get_topk_closest_indice(q_windows, k_windows, topk=1):
    ...
    if q_windows[0] != k_windows[0]:
        ...
        # print("coords_h_q: {}".format(coords_h_q), flush=True)
        # print("coords_w_q: {}".format(coords_w_q), flush=True)
    else:
        factor = 1
...
class WindowAttention(nn.Cell):
    ...
    def __init__(self, dim, input_resolution, expand_size, shift_size, window_size, window_size_glo,
        ...
        # print("nWh, nWw: {} {}".format(self.nWh, self.nWw), flush=True)
        ...
        for k in range(self.focal_level):
            ...
            # print("topk_closest_indice:\n{}".format(topk_closest_indice), flush=True)
            # print("topk_closest_indice.shape: {}".format(topk_closest_indice.shape), flush=True)
        ...
        self.topk_cloest_indice_param_0 = self.topk_cloest_indice_params[0]
        self.topk_cloest_indice_param_1 = self.topk_cloest_indice_params[1]
        self.topk_cloest_coords_param_0 = self.topk_cloest_coords_params[0]
        self.topk_cloest_coords_param_1 = self.topk_cloest_coords_params[1]
        self.topk_closest_indice_shape_0 = self.topk_closest_indice_shapes[0]
        self.topk_closest_indice_shape_1 = self.topk_closest_indice_shapes[1]
    ...
    def construct(self, x_all, mask_all=None):
        ...
        """
        # print("shape 0: {}".format(x_all[0].shape), flush=True)
        # print("shape 1: {}".format(x_all[1].shape), flush=True)
        ...
        # focal level 0
        topk_closest_indice_shape_0 = self.topk_closest_indice_shape_0
        topk_closest_indice_0 = self.topk_cloest_indice_param_0
        topk_indice_k_0 = self.reshape(topk_closest_indice_0, (1, -1))
        topk_indice_k_0 = numpy.tile(topk_indice_k_0, (B, 1))

        topk_coords_k_0 = self.topk_cloest_coords_param_0
        topk_rpb_k_0 = self.coord2rpb_all[0](topk_coords_k_0)
        topk_rpbs.append(topk_rpb_k_0)

        k_k_0 = self.reshape(k, (B, -1, self.num_heads, C // self.num_heads))
        v_k_0 = self.reshape(v, (B, -1, self.num_heads, C // self.num_heads))

        topk_indice_k_0 = self.reshape(topk_indice_k_0, (B, -1, 1, 1))
        topk_indice_k_0 = numpy.tile(topk_indice_k_0, (1, 1, self.num_heads, C // self.num_heads))
        k_k_selected_0 = self.gather(k_k_0, 1, topk_indice_k_0)
        v_k_selected_0 = self.gather(v_k_0, 1, topk_indice_k_0)

        # [B, x, y, num_heads, C // num_heads]
        selected_shape_0_0 = (B,) + topk_closest_indice_shape_0 + (self.num_heads, C // self.num_heads)
        selected_shape_1_0 = (-1, self.num_heads, topk_closest_indice_shape_0[1], C // self.num_heads)

        # print("k_k_selected shape: {}".format(k_k_selected.shape), flush=True)
        # print("k_k_selected reshape: {}".format(selected_shape_0), flush=True)
        k_k_selected_0 = self.reshape(k_k_selected_0, selected_shape_0_0)
        k_k_selected_0 = self.transpose(k_k_selected_0, (0, 1, 3, 2, 4))
        v_k_selected_0 = self.reshape(v_k_selected_0, selected_shape_0_0)
        v_k_selected_0 = self.transpose(v_k_selected_0, (0, 1, 3, 2, 4))

        k_k_selected_0 = self.reshape(k_k_selected_0, selected_shape_1_0)
        v_k_selected_0 = self.reshape(v_k_selected_0, selected_shape_1_0)

        k_all.append(k_k_selected_0)
        v_all.append(v_k_selected_0)
        attn_cat_shape += topk_closest_indice_shape_0[1]
        # focal level 0

        # focal level 1
        topk_closest_indice_shape_1 = self.topk_closest_indice_shape_1
        topk_closest_indice_1 = self.topk_cloest_indice_param_1
        topk_indice_k_1 = self.reshape(topk_closest_indice_1, (1, -1))
        topk_indice_k_1 = numpy.tile(topk_indice_k_1, (B, 1))

        topk_coords_k_1 = self.topk_cloest_coords_param_1
        topk_rpb_k_1 = self.coord2rpb_all[1](topk_coords_k_1)
        topk_rpbs.append(topk_rpb_k_1)

        x_k_1 = x_all[1]
        qkv_k_1 = self.qkv(x_k_1)
        qkv_k_1 = self.reshape(qkv_k_1, (B, -1, 3, self.num_heads, C // self.num_heads))
        q_k_1, k_k_1, v_k_1 = self.split(qkv_k_1)
        k_k_1 = self.squeeze(k_k_1)
        # print("k_k_1 shape: {}".format(k_k_1.shape), flush=True)
        v_k_1 = self.squeeze(v_k_1)

        topk_indice_k_1 = self.reshape(topk_indice_k_1, (B, -1, 1, 1))
        # print("topk_indice_k_1 shape old: {}".format(topk_indice_k_1.shape), flush=True)
        topk_indice_k_1 = numpy.tile(topk_indice_k_1, (1, 1, self.num_heads, C // self.num_heads))
        # print("topk_indice_k_1 shape new: {}".format(topk_indice_k_1.shape), flush=True)
        k_k_selected_1 = self.gather(k_k_1, 1, topk_indice_k_1)
        # print("k_k_selected_1 shape: {}".format(k_k_selected_1.shape), flush=True)
        v_k_selected_1 = self.gather(v_k_1, 1, topk_indice_k_1)

        # [B, x, y, num_heads, C // num_heads]
        selected_shape_0_1 = (B,) + topk_closest_indice_shape_1 + (self.num_heads, C // self.num_heads)
        selected_shape_1_1 = (-1, self.num_heads, topk_closest_indice_shape_1[1], C // self.num_heads)

        # print("k_k_selected shape: {}".format(k_k_selected.shape), flush=True)
        # print("k_k_selected reshape: {}".format(selected_shape_0), flush=True)
        k_k_selected_1 = self.reshape(k_k_selected_1, selected_shape_0_1)
        k_k_selected_1 = self.transpose(k_k_selected_1, (0, 1, 3, 2, 4))
        v_k_selected_1 = self.reshape(v_k_selected_1, selected_shape_0_1)
        v_k_selected_1 = self.transpose(v_k_selected_1, (0, 1, 3, 2, 4))

        k_k_selected_1 = self.reshape(k_k_selected_1, selected_shape_1_1)
        v_k_selected_1 = self.reshape(v_k_selected_1, selected_shape_1_1)

        k_all.append(k_k_selected_1)
        v_all.append(v_k_selected_1)
        attn_cat_shape += topk_closest_indice_shape_1[1]
        # focal level 1
